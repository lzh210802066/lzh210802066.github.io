...
浏览器的样式优先级是：
1. 行内样式（style="..."） → 优先级最高
2. 外部 / 内联 CSS 规则（比如你写的 img { ... }）
3. HTML 属性（height="..." / width="..."）


为什么用 <p> 框起来？

HTML 中 <img> 是一个 行内元素（inline element），它本身并不是块级元素（block-level element）。

行内元素默认是靠左排列，不能直接用 align="center" 让它水平居中。

1解决方法之一：把图片放在一个 块级容器里，然后设置居中。<p> 标签就是常用的块级容器。
